 ...
 */
async function initializeNFTBeater(connection: Connection, wallet: Keypair) {
  console.log("Initializing NFT Beater account...");
  ...
  console.log("Account not found, creating initialization instruction...");
  
  // Create instruction data (0 is the instruction discriminator for initialize)
  const instructionData = Buffer.from([0]);
  
  // Create the instruction for initializing the account
  const instruction = new TransactionInstruction({
    keys: [
      { pubkey: nftBeaterPDA, isSigner: false, isWritable: true },
      { pubkey: wallet.publicKey, isSigner: true, isWritable: true },
      { pubkey: MERKLE_TREE_ADDRESS, isSigner: false, isWritable: false },
...
async function uploadRarityData() {
  console.log("Starting direct rarity data upload...");
  ...
  try {
    ...
    // Initialize NFT Beater account first
    const nftBeaterPDA = await initializeNFTBeater(connection, wallet);
    ...
    const chunkSize = 500; // Smaller chunks for direct transactions
